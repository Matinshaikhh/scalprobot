...
//--- These two live here rather than beside CRunProvenance because the
//--- EA declares an `input` of the segment type and the configuration
//--- layer carries it, and neither may depend on Runtime\.

//--- The tester's tick-generation mode, as INFERRED from the tape.
//--- MQL5 exposes no API for it, so it is measured. See CRunProvenance.
enum ENUM_SRP_TICK_MODEL
  {
   SRP_TICK_MODEL_UNKNOWN=0,     // too few bars observed to judge
   SRP_TICK_MODEL_LIVE,          // not a tester pass at all
   SRP_TICK_MODEL_OPEN_PRICES,   // one tick per bar
   SRP_TICK_MODEL_OHLC_M1,       // a handful per bar, constant spread
   SRP_TICK_MODEL_GENERATED,     // many per bar, still constant spread
   SRP_TICK_MODEL_REAL_TICKS     // many per bar, spread varies within it
  };

//--- Which half of the data a run is allowed to speak for.
//---
//--- UNDECLARED is the default ON PURPOSE. A run that does not say which
//--- segment it used must read as undeclared in its own header rather
//--- than inheriting a flattering label by omission.
enum ENUM_SRP_DATA_SEGMENT
  {
   SRP_SEGMENT_UNDECLARED=0,
   SRP_SEGMENT_DEV,              // development / in-sample
   SRP_SEGMENT_OOS               // out-of-sample validation
  };

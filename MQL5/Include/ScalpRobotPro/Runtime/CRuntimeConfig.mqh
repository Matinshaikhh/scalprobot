...
struct SRuntimeConfig
  {
   ...
   //--- HARNESS v2: TRUE = a daily-loss breach latches for the whole run
   //--- (legacy behaviour). FALSE = it stands the EA down until the next
   //--- session and then rearms. Drawdown and equity-floor breaches are
   //--- terminal either way.
   ...
   //--- HARNESS v2, FIX 4. REPORTING ONLY. Which half of the data this run
   //--- is allowed to speak for, as declared by the operator. Held as an
   //--- int like every other enum in this struct so it round-trips through
   //--- the keyed config store. Nothing on the trading path reads it: it
   //--- reaches the report header and stops there.
   int               data_segment;

   //--- HARNESS v2, FIX 5. THE OPPOSITE OF THE FIELD ABOVE: this one is on
   //--- the trading path. Points; 0.0 means "read SYMBOL_SPREAD at init",
   //--- which is what every run in this project's history did.
   //---
   //--- Audit 5.5 found the stop floor, the fixed stop and target, the
   //--- break-even trigger and offset, the trailing start, distance and
   //--- step, the spread ceiling, the slippage allowance, the order
   //--- deviation, the gap threshold and the structure-break buffer all
   //--- computed from one spread reading and never revisited. Pinning the
   //--- reading is what lets two runs over the same dates be the same
   //--- experiment; it does not make the geometry any less static.
   ...
   void              Reset(void)
     {
      ...
      bos_enabled=false; volatility_breakout_enabled=true;  // BoS off by default, see CMarketProfile
      ...
      scalp_commission_points=6.0;      // HARNESS v2: modelled $6/lot round turn
      ...
      //--- HARNESS v2, FIX 4. Undeclared until an operator says otherwise.
      data_segment=(int)SRP_SEGMENT_UNDECLARED;
      //--- HARNESS v2, FIX 5. 0.0 = live sample. The default has to be the
      //--- historical behaviour or this fix would itself change every
      //--- number it exists to make comparable.
      pin_spread_sample=0.0;
      ...
      //--- Tier fallbacks. These are what a MISSING config key resolves to,
      //--- so they must match CConfigurationBuilder and the EA inputs or the
      //--- three disagree silently. Widened 2026-09-05 with the others; the
      //--- old SUPER row (0.30 ATR target, 0.72 assumed win rate) was the
      //--- worst of the three copies - at ATR~238 that target is ~71pts
      //--- against a ~46.5pt round-trip cost, i.e. cost was 65% of gross.
      tier_enabled[0]=true; tier_target_atr[0]=1.35;
      tier_stop_atr[0]=0.60; tier_hold_seconds[0]=600;
      tier_win_rate[0]=0.47; tier_timeframe[0]=(int)PERIOD_M1;
      tier_enabled[1]=true; tier_target_atr[1]=1.20;
      tier_stop_atr[1]=0.60; tier_hold_seconds[1]=300;
      tier_win_rate[1]=0.50; tier_timeframe[1]=(int)PERIOD_M1;
      tier_enabled[2]=true; tier_target_atr[2]=1.60;
      tier_stop_atr[2]=0.75; tier_hold_seconds[2]=1800;
      tier_win_rate[2]=0.45; tier_timeframe[2]=(int)PERIOD_M5;
...
bool CRuntimeConfig::Load(IConfigProvider *config,
                          ...
                          SRuntimeConfig &out)
  {
   ...
   out.daily_limit_terminal = config.GetBool(CConfigKeys::GUARD_DAILY_LIMIT_TERMINAL,d.daily_limit_terminal);
   ...
   out.data_segment                = config.GetInt(CConfigKeys::GENERAL_DATA_SEGMENT,d.data_segment);
   out.pin_spread_sample           = config.GetDouble(CConfigKeys::GENERAL_PIN_SPREAD_SAMPLE,d.pin_spread_sample);
...
string CRuntimeConfig::Describe(const SRuntimeConfig &config)
  {
   ...
   text+=" flattenOnTrip="+(config.flatten_on_trip ? "y" : "n");
   text+=" dailyLimitTerminal="+(config.daily_limit_terminal ? "y" : "n");
   ...
   //--- HARNESS v2, FIX 4. Named, not numbered, and printed in the startup
   //--- config dump as well as the report header: the one place an operator
   //--- can catch a mislabelled run before it has produced a number.
   text+="\n  segment="+EnumToString((ENUM_SRP_DATA_SEGMENT)config.data_segment);
   //--- HARNESS v2, FIX 5. Printed beside the segment because the two
   //--- together are what decide whether a number from this run can be
   //--- quoted and repeated.
   text+="  spreadPin="+(config.pin_spread_sample>0.0
                         ? StringFormat("%.2f pts",config.pin_spread_sample)
                         : "live");
   return(text);
  }

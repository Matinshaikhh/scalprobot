...
//--- HARNESS v2, FIX 4. Passive observer of the tape and of the run's own
//--- declarations. It holds no pointer to anything that can trade.
#include "CRunProvenance.mqh"
...
private:
   ...
   bool                   m_built;
   bool                   m_running;
   //--- HARNESS v2. A DAILY stand-down is not a halt: it suspends new
   //--- entries for the remainder of the session and rearms when the guard
   //--- rolls its day window. Kept distinct from m_running's operator halt
   //--- and from the emergency latch so that a routine losing day does not
   //--- silently end a multi-month backtest. m_stand_down_day records the
   //--- guard's day_start at the moment of the trip, which is what the
   //--- rearm test compares against - not wall-clock time, so it behaves
   //--- identically in the tester and live.
   bool                   m_stood_down;
   datetime               m_stand_down_day;
   long                   m_stand_down_count;
   ...
   //=== HARNESS v2: SERVER-SIDE EXIT QUALITY ==========================
   //--- Aggregates over the exits the EA did NOT send: the broker firing a
   //--- stop or a target. The pre-existing slippage accounting covered only
   //--- EA-sent orders, i.e. entries and manual closes, which on a scalping
   //--- system is the minority of fills and the flattering minority - a
   //--- server-side stop fill is exactly where a gap costs money. Sign
   //--- convention throughout: POSITIVE IS ADVERSE, longs and shorts alike.
   //--- On an OHLC-modelled run these sums are zero BY CONSTRUCTION, which
   //--- is not good news but a diagnostic: it is the tester filling every
   //--- stop at the stop price. Fix 4 stamps them into the report header so
   //--- that fact can never be read as a result.
   long                   m_srv_exits;          // server-fired exits seen
   long                   m_srv_sl_exits;       // ... of which stops
   long                   m_srv_tp_exits;       // ... of which targets
   double                 m_srv_slip_sum;       // sum of signed points
   double                 m_srv_slip_worst;     // worst single adverse points
   double                 m_srv_slip_money;     // sum of signed money

   //=== HARNESS v2, FIX 4: RUN PROVENANCE =============================
   //--- Held BY VALUE, not by pointer, and deliberately so: it must exist
   //--- before Build() succeeds and survive until after Release(), because
   //--- the two moments a report is written are a build failure and a
   //--- shutdown. A pointer would have to be allocated in one of those
   //--- windows and checked for NULL in both.
   CRunProvenance         m_prov;
   //--- SYMBOL_POINT, resolved once at Start. The observer runs on every
   //--- tick before the throttle, so it is the one place in this class
   //--- where a per-tick SymbolInfoDouble call is worth avoiding.
   double                 m_point;
...
public:
   ...
   //--- HARNESS v2. The non-terminal counterpart of EmergencyFlatten:
   //--- stops new entries, optionally closes what is open, and leaves the
   //--- emergency latch UNARMED so the next session can trade. Open
   //--- positions stay managed either way.
   int               StandDownForDay(const string reason,const bool flatten);
   //--- Rearms a daily stand-down once the guard has rolled its day
   //--- window. Cheap, and called on the tick path because the tester
   //--- never fires OnTimer.
   void              RearmIfNewSession(void);
   bool              IsStoodDown(void) const { return(m_stood_down); }
   long              StandDownCount(void) const { return(m_stand_down_count); }

   //=== HARNESS v2: SERVER-SIDE EXIT QUALITY ==========================
   //--- Folds one broker-fired exit into the aggregates. Pure bookkeeping:
   //--- it is called from the trade-transaction handler AFTER the trade has
   //--- been recorded and it influences no decision.
   void              RecordServerExit(const bool by_stop,const double points,
                                     const double money);
   //--- One line for the report header. Reports the count, the split, the
   //--- mean and worst adverse points and the money, plus an explicit
   //--- warning when the count is zero on a run that had server-side exits,
   //--- because that is the OHLC-model signature rather than clean fills.
   string            ServerExitSummary(void) const;
   //--- True when the run had server-side exits and every one of them filled
   //--- at exactly its trigger price - the generated-tape signature.
   bool              ServerFillsAllExact(void) const;
   long              ServerExitCount(void) const { return(m_srv_exits); }
   double            ServerExitSlippageAvg(void) const
     { return(m_srv_exits>0 ? m_srv_slip_sum/(double)m_srv_exits : 0.0); }
   double            ServerExitSlippageWorst(void) const { return(m_srv_slip_worst); }
   double            ServerExitSlippageMoney(void) const { return(m_srv_slip_money); }

   //=== HARNESS v2, FIX 4: RUN PROVENANCE =============================
   //--- The block that opens every report: inferred tick model, measured
   //--- spread, declared cost model, declared data segment, and the
   //--- server-side exit quality that only means anything once the tick
   //--- model beside it is known. Read-only; it decides nothing.
   string            ProvenanceHeader(void) const;
   //--- Hands fix 2's final server-fill counts to the provenance object.
   //--- Must run before a header is printed or a file is written; a count
   //--- taken at Start() would be zero on every run by definition.
   void              DeclareRunEvidence(void);
   //--- The same facts on one comma-free line, for the first line of a
   //--- data file. See CRunProvenance::HeaderCsv.
   string            ProvenanceCsv(void) const { return(m_prov.HeaderCsv()); }
   bool              IsRunQuotable(void) const { return(m_prov.IsQuotable()); }
...
CProductionEngine::CProductionEngine(void)
  : m_clock(NULL),m_bus(NULL),m_env(NULL),
    ...
    m_stood_down(false),m_stand_down_day(0),m_stand_down_count(0),
    m_halt_reason(""),
    m_gate_pre_risk(0),m_gate_risk_rating(0),m_gate_direction(0),
    m_gate_accuracy(0),m_gate_scalp(0),m_gate_risk_engine(0),
    m_srv_exits(0),m_srv_sl_exits(0),m_srv_tp_exits(0),
    m_srv_slip_sum(0.0),m_srv_slip_worst(0.0),m_srv_slip_money(0.0),
...
bool CProductionEngine::Build(IConfigProvider *config,
                              ...
                              const ENUM_TIMEFRAMES timeframe)
  {
   ...
   //--- HARNESS v2, FIX 4. Identity and the point size, resolved here rather
   //--- than in Start so that the tape observer is usable the moment the
   //--- first tick can arrive, whatever order the caller chooses. Start
   //--- adds the declarations - cost model and data segment - and prints.
   m_point=SymbolInfoDouble(symbol,SYMBOL_POINT);
   m_prov.Configure(symbol,timeframe);
   ...
   //--- HARNESS v2, FIX 5. THE SECOND SAMPLE, AND THE REASON THE PIN IS NOT
   //--- OPTIONAL HERE.
   //---
   //--- This Resolve runs later than the entry point's, so on a live feed it
   //--- reads a DIFFERENT spread from the one the sealed configuration's
   //--- geometry was derived from. The regime and volatility bands the engine
   //--- classifies against are therefore built from one tick and the stops
   //--- from another, and nothing in any log said so.
   //---
   //--- Pinning both sites makes them agree by construction. The double
   //--- resolve is deliberately left in place: collapsing it to one shared
   //--- struct would change which numbers the regime engine sees, and that
   //--- is a trading decision, which this fix is not permitted to move.
   CSymbolClassifier::PinSpreadSample(symbol_spec,m_config.pin_spread_sample);
...
bool CProductionEngine::BuildRisk(void)
  {
   ...
   if(m_limits==NULL)
      return(false);
   //--- HARNESS v2. flatten_on_trip answers "close what is open?" and is
   //--- legitimately the same answer for both limits. It does NOT answer
   //--- "is the run over?" - that is daily_limit_terminal, and it is
   //--- applied where the breach is REACTED to, in SeekEntry, because
   //--- terminality is an action policy and this class only detects.
...
bool CProductionEngine::BuildDecision(void)
  {
   ...
   //--- HARNESS v2, FIX 5. Ordered deliberately: this must reach the
   //--- controller before m_scalp.Initialize() runs further down, because
   //--- Initialize() is where the sample is taken and the three cost
   //--- distances are derived from it. Called after ConfigureCosts so an
   //--- explicitly configured execution cost still wins over a derived one -
   //--- the pin changes what "derived" derives FROM, not whether it happens.
   m_scalp.ConfigureSpreadPin(m_config.pin_spread_sample);
   ...
   //--- REDESIGN 2026-09-04. The sample the break-even gate needs before a
   //--- MEASURED win rate is allowed to displace the configured one. Shares
   //--- InpKellyMinTrades because it is the same question - how many closed
   //--- trades before a rate means anything - and having two different
   //--- answers to it in one robot would be worse than either.
   m_scalp.SetWinRateMinSample(m_config.kelly_min_trades);
...
int CProductionEngine::Start(void)
  {
   ...
   //=== HARNESS v2, FIX 4 ============================================
   //--- Declare the run before it produces a single number. Three things
   //--- go in: what instrument, what the run charges itself, and which half
   //--- of the data it claims to speak for. The fourth - the tick model -
   //--- is not declared but MEASURED, and is therefore unavailable until
   //--- the tape has been observed. That is why the header is printed twice:
   //--- once here with the declarations, and once at Stop with the verdict.
   m_point=SymbolInfoDouble(m_config.symbol,SYMBOL_POINT);
   m_prov.Configure(m_config.symbol,m_config.timeframe);
   //--- THE COST MODEL IS TAKEN FROM THE OBJECT THAT ENFORCES IT.
   //--- CScalpController::Initialize() has already run by now (Build precedes
   //--- Start), so these are the EFFECTIVE numbers: the ones left at zero in
   //--- the configuration have been filled in from its single startup reading
   //--- of SYMBOL_SPREAD. Reading m_config here instead would print "target
   //--- floor 0.00" for the common case where the operator let the code derive
   //--- it, describing a run that did not happen; re-reading SYMBOL_SPREAD
   //--- here would answer a different question, since the spread has moved on
   //--- by several ticks and on XAUUSD can differ by a factor of three.
   //--- Pinning that chain and logging all twelve derived values is fix 5;
   //--- reporting the one number they all descend from, and flagging it when
   //--- the run's own tape disagrees with it, is fix 4.
   if(m_scalp!=NULL)
      m_prov.SetCostModel(m_scalp.InitSpreadPoints(),
                          m_scalp.CommissionPoints(),
                          m_scalp.ExecutionCostPoints(),
                          m_scalp.MinRewardCostRatio(),
                          m_scalp.TargetMinPoints(),
                          m_scalp.EarlyExitMinPoints());
   else
      //--- No controller means no scalp geometry to report. The configured
      //--- values are still declared, so the header states a cost model
      //--- rather than UNDECLARED, and the startup spread is read here
      //--- because nothing else sampled it.
      m_prov.SetCostModel((double)SymbolInfoInteger(m_config.symbol,
                                                    SYMBOL_SPREAD),
                          m_config.scalp_commission_points,
                          m_config.scalp_execution_cost_points,
                          m_config.scalp_min_reward_cost_ratio,
                          m_config.scalp_target_min_points,
                          m_config.scalp_early_exit_min_points);
   m_prov.SetSegment((ENUM_SRP_DATA_SEGMENT)m_config.data_segment);
   //--- HARNESS v2, FIX 5. THE ROOT OF THE GEOMETRY, DECLARED.
   //---
   //--- Taken from the market profile the engine actually built, so the count
   //--- is what the preset LEFT to be derived rather than what could be. The
   //--- pinned flag comes from the same struct: SSymbolProfile carried it
   //--- through PinSpreadSample and ScaleToSymbol copied it beside the sample
   //--- it was used for, which is the only arrangement in which the two
   //--- cannot drift apart.
   //---
   //--- WHICH SAMPLE THIS IS, precisely. It is the ENGINE's, from the second
   //--- Resolve. Pinned, that is the same number the entry point derived the
   //--- sealed configuration's distances from. Unpinned, the tester returns
   //--- the same value at both sites because no tick arrives inside a single
   //--- OnInit, but on a live feed SYMBOL_SPREAD moves asynchronously and the
   //--- two can differ - one more reason the unpinned case is reported as
   //--- non-reproducible rather than merely unpinned.
   m_prov.SetGeometry(m_market_profile.spread_sample,
                      m_market_profile.spread_pinned,
                      CMarketProfileFactory::DerivedCount(m_market_profile));
   //--- The full derivation table, once, beside the header that summarises it.
   //--- The header states whether the run is reproducible; this states what
   //--- would have to be reproduced.
   Print(CMarketProfileFactory::DescribeDerivation(m_market_profile));
   //--- Printed, not logged. A provenance header that a muted log level or a
   //--- suppressed log file can hide is not a provenance header.
   Print(ProvenanceHeader());
...
void CProductionEngine::Stop(const int deinit_reason)
  {
   ...
   //=== HARNESS v2, FIX 4 ============================================
   //--- The header goes ABOVE the numbers, not below them. By now the tape
   //--- has been observed, so this copy carries the MEASURED tick model and
   //--- the run's final fill evidence rather than the declarations alone -
   //--- and a reader scrolling a journal meets the conditions before the
   //--- figures those conditions qualify. Printed even on a zero-tick run,
   //--- because "0 ticks observed" is itself the reason the numbers below
   //--- are empty.
   DeclareRunEvidence();
   Print(ProvenanceHeader());
   if(!IsRunQuotable())
      Print(SRP_PRODUCT_NAME,": THE FIGURES BELOW MAY NOT BE QUOTED. See the"
            " warning list in the provenance block above.");
   ...
   if(m_ticks>0)
     {
      ...
      if(m_scalp!=NULL && m_scalp.IsEnabled())
        {
         //--- HARNESS v2, FIX 4. The cost model FIRST, because the funnel's
         //--- COST refusal count is unreadable without it: a run that refused
         //--- 600 signals on cost either faced a genuinely uneconomic tape or
         //--- charged itself a floor derived from one unrepresentative spread
         //--- reading, and only this line distinguishes the two.
         Print("  ",m_scalp.DescribeCostModel());
   ...
   if(m_ui!=NULL)
     {
      //--- HARNESS v2, FIX 4. Handed over BEFORE anything is written, so
      //--- every artefact this block produces carries the same provenance
      //--- line as the journal block above. A sidecar file was the other
      //--- option and was rejected: a sidecar can be separated from the
      //--- numbers, which is the precise failure this fix exists to stop.
      m_ui.SetReportPreamble(ProvenanceCsv());
...
void CProductionEngine::OnTickEvent(void)
  {
   ...
   //--- 0. HARNESS v2, FIX 4. Measure the tape BEFORE the throttle, and
   //--- before any filter. The throttle is disabled in the tester and
   //--- active live, so observing after it would make the tick model
   //--- inference a property of the throttle setting rather than of the
   //--- feed - and the whole purpose of the measurement is to be
   //--- independent of what the EA chooses to do with the ticks. Bounded
   //--- work: two integer bumps and a scan over at most 64 entries.
   MqlTick tick;
   if(SymbolInfoTick(m_config.symbol,tick))
      m_prov.ObserveTick(tick,m_point);
   ...
   RefreshAccountState();
   //--- HARNESS v2. RefreshAccountState has just rolled the guard's day
   //--- window (CRiskEngine::UpdateAccountState -> UpdateWindows), so this
   //--- is the first moment a new session is observable. It lives on the
   //--- tick path and not in OnTimerEvent because the tester never arms
   //--- the timer, and a rearm that only works live is not a rearm.
   RearmIfNewSession();
...
ENUM_SRP_SCALP_TIER CProductionEngine::SelectScalpTier(void) const
  {
   ...
   //---
   //--- REDESIGN 2026-09-04. THAT TABLE LEFT SWING UNREACHABLE, AND SO LEFT
   //--- THE ROBOT WITH ONE TIER.
   //---
   //--- EXTREME volatility was the only route into SWING, and
   //--- CRegimeEngine::ApplyAdaptation sets trading_allowed=false for
   //--- exactly that class - "dislocation, stand aside". Every run since
   //--- has therefore reported SWING_SCALP on entries=0 while every trade
   //--- went to SUPER, whose 0.65xATR target could not clear a 39-47 point
   //--- round-trip cost at any win rate the tier could plausibly reach.
   //--- Widening SUPER is half the answer; the other half is that the wider
   //--- geometry has to be reachable in conditions the robot is allowed to
   //--- trade in.
   //---
   //--- Routing now follows the VOLATILITY CLASS, because that is what
   //--- decides whether a given ATR multiple is reachable at all, while
   //--- regime continues to decide whether to trade via trading_allowed:
   //---
   //---   LOW / NORMAL  -> SUPER. Modest ATR; a 1.6xATR target here is a
   //---     swing position taken in a quiet market, and the ATR floor
   //---     refuses the genuinely dead cases anyway.
   //---   HIGH          -> SWING. Larger ATR means more travel before
   //---     stalling, so the wide geometry is both reachable and the one
   //---     whose collected target dwarfs the fixed part of the cost.
   //---   EXTREME       -> SWING, retained for correctness. The regime
   //---     engine stands the robot down before this is used; if an
   //---     operator ever allows EXTREME, the wide geometry is the right
   //---     one rather than a scalp target inside one bar's range.
   //---
   //--- The point of this shape is that BOTH tiers are now reachable, so a
   //--- walk-forward can measure them against each other instead of
   //--- reporting one of them as permanently idle.
   switch(state.volatility)
     {
      ...
      case SRP_VOL_NORMAL:
      default:
         tier=(state.regime==SRP_REGIME_TRANSITION ? SRP_TIER_STANDARD
                                                   : SRP_TIER_SUPER);
         break;
     }
...
void CProductionEngine::SeekEntry(const bool is_new_bar)
  {
   ...
   //--- THE SIGNAL'S OWN INVALIDATION LEVEL, IN POINTS.
   //---
   //--- REDESIGN 2026-09-04. SStrategySignal.suggested_stop has existed
   //--- since Phase 3 and was read by nothing outside a test script.
   //--- CLiquiditySweepStrategy sets it to the sweep extreme - the price
   //--- at which the fade thesis is dead - and the robot then stopped out
   //--- at 0.45xATR instead, a distance with no relationship to where the
   //--- trade was wrong. Converted here, validated inside the scalp gate,
   //--- and applied to the protection manager below so sizing, the
   //--- break-even test and the order all use one stop.
   //---
   //--- Signals whose level sits on the wrong side of price are discarded
   //--- rather than flipped: a stop through the entry means the level is
   //--- stale, not that the trade wants a target there.
   double structural_stop_points=0.0;
   if(decision.signal.suggested_stop>0.0)
     {
      const double sp=SymbolInfoDouble(m_config.symbol,SYMBOL_POINT);
      if(sp>0.0)
        {
         const double dist=(is_buy ? entry-decision.signal.suggested_stop
                                   : decision.signal.suggested_stop-entry);
         if(dist>0.0)
            structural_stop_points=dist/sp;
        }
     }
   ...
   if(scalping)
     {
      ...
      if(!m_scalp.Evaluate(scalp_id,TimeCurrent(),spread_points,
                           ...
                           scalp_tier,structural_stop_points))
        {
         m_gate_scalp++;
   ...
   if(scalping && scalp_verdict.allowed && m_protection!=NULL)
     {
      //--- REDESIGN 2026-09-04. When the gate accepted the signal's own
      //--- invalidation level, the order must carry THAT stop and volume
      //--- must be computed from it. Handing the protection manager an ATR
      //--- multiple here while the break-even test priced a structural
      //--- stop would size the position against one distance and place it
      //--- at another - the exact class of mismatch that made 48 of 48
      //--- trades run a stop their tier was never costed for.
      if(scalp_verdict.stop_is_structural && scalp_verdict.stop_points>0.0)
         m_protection.ConfigureStop(SRP_STOP_FIXED_POINTS,
                                    scalp_verdict.stop_points,
                                    0.0);
      else
        {
   ...
   if(!m_risk.EvaluateEntry(is_buy,entry,TotalOpenLots(),
                            ExposureRiskPercent(),sizing,plan,verdict))
     {
      ...
      //---
      //--- HARNESS v2: BUT NOT EVERY BREACH IS TERMINAL. The old code sent
      //--- every flatten-requiring verdict to EmergencyFlatten, which arms
      //--- two latches that never auto-clear (engine halt + guard
      //--- emergency). A 2% losing day therefore ended the run outright,
      //--- and every backtest in this project's history stopped at its
      //--- first bad day - which flatters the surviving statistics, because
      //--- the run is truncated exactly where the strategy was performing
      //--- worst. Route by breach type instead:
      //---   DAILY / WEEKLY / MONTHLY -> stand down, rearm next window
      //---   DRAWDOWN / EMERGENCY     -> terminal, unchanged
      //--- The daily case can still be made terminal with
      //--- InpDailyLimitTerminal for anyone who wants the old behaviour.
      if(verdict.flatten_required)
        {
         const bool terminal=
            (verdict.breach==SRP_BREACH_MAX_DRAWDOWN ||
             verdict.breach==SRP_BREACH_EMERGENCY ||
             (verdict.breach==SRP_BREACH_DAILY_LOSS &&
              m_config.daily_limit_terminal));
         if(terminal)
            EmergencyFlatten("risk guard: "+verdict.detail);
         else
            StandDownForDay("risk guard: "+verdict.detail,true);
        }
      return;
     }
...
void CProductionEngine::BuildDashboardModel(SDashboardModel &model)
  {
   ...
   //--- HARNESS v2. A stand-down reads as HALTED above, which is true but
   //--- not informative: the operator needs to know it clears by itself.
   if(m_stood_down)
      model.engine_state="STOOD DOWN (rearms next session): "+m_halt_reason;
...
void CProductionEngine::OnTradeTransactionEvent(const MqlTradeTransaction &transaction,
                                                ...
                                                const MqlTradeResult &result)
  {
   ...
   //=== HARNESS v2: SLIPPAGE ON SERVER-SIDE EXITS ====================
   //---
   //--- DEAL_REASON is the broker's own statement of why the position went;
   //--- it does not depend on parsing a comment string, which is what the
   //--- rest of this method has to do and which differs per broker.
   //--- DEAL_SL / DEAL_TP carry the levels that were in force ON THIS DEAL,
   //--- so they remain readable after the position has ceased to exist.
   //---
   //--- Sign convention: POSITIVE = ADVERSE. For a long, an adverse fill is
   //--- BELOW the trigger; for a short it is ABOVE. Applying that to both
   //--- stops and targets means one number answers "did the exit cost me
   //--- anything?" regardless of side or exit type, and the aggregate can be
   //--- summed without cancelling.
   trade.close_reason=(int)HistoryDealGetInteger(transaction.deal,DEAL_REASON);
   const bool by_stop  =(trade.close_reason==(int)DEAL_REASON_SL);
   const bool by_target=(trade.close_reason==(int)DEAL_REASON_TP);
   if(by_stop || by_target)
     {
      trade.server_side_exit=true;
      trade.exit_trigger_price=(by_stop
                                ? HistoryDealGetDouble(transaction.deal,DEAL_SL)
                                : HistoryDealGetDouble(transaction.deal,DEAL_TP));
      const double point=SymbolInfoDouble(m_config.symbol,SYMBOL_POINT);
      if(trade.exit_trigger_price>0.0 && point>0.0 && trade.close_price>0.0)
        {
         const double adverse=(trade.is_buy
                               ? trade.exit_trigger_price-trade.close_price
                               : trade.close_price-trade.exit_trigger_price);
         trade.exit_slippage_points=adverse/point;
         if(m_risk!=NULL && trade.volume>0.0)
           {
            const double per_lot=
               m_risk.BrokerMoneyPerLot(MathAbs(trade.exit_slippage_points));
            if(per_lot>0.0)
               trade.exit_slippage_money=
                  (trade.exit_slippage_points>=0.0 ? 1.0 : -1.0)*
                  per_lot*trade.volume;
           }
        }
      RecordServerExit(by_stop,trade.exit_slippage_points,
                       trade.exit_slippage_money);
      CEnterpriseLogger *exec_log=m_ui.Logger();
      if(exec_log!=NULL)
        {
         const int digits=(int)SymbolInfoInteger(m_config.symbol,SYMBOL_DIGITS);
         exec_log.LogTrade("ServerFill",
                           StringFormat("%s fired: trigger=%s fill=%s "
                                        "slip=%+.1fpts (%+.2f) [+ = adverse]",
                                        (by_stop ? "STOP" : "TARGET"),
                                        DoubleToString(trade.exit_trigger_price,digits),
                                        DoubleToString(trade.close_price,digits),
                                        trade.exit_slippage_points,
                                        trade.exit_slippage_money),
                           trade.ticket,trade.close_price,
                           trade.volume,trade.net_profit);
         //--- Same EXECUTION CHANNEL the EA-sent orders write to via
         //--- LogExecutionTime, so one CSV finally holds every fill rather
         //--- than only the ones the EA initiated. Note the dashboard's
         //--- "average slippage" still reads m_trade.AverageSlippagePoints(),
         //--- which is EA-sent only by construction; the combined figure
         //--- lives in ServerExitSummary() and in this channel, not there.
         exec_log.LogExecutionTime((by_stop ? "ServerStopFill"
                                            : "ServerTargetFill"),
                                   0.0,0,trade.exit_slippage_points,
                                   trade.ticket);
        }
     }
...
double CProductionEngine::OnTesterEvent(void)
  {
   ...
   //=== HARNESS v2, FIX 4 ============================================
   //--- OnTester runs after the last tick, so the tape has been fully
   //--- observed and the tick-model verdict here is final. This is the most
   //--- load-bearing place in the whole fix: the number returned below is
   //--- what an optimiser SORTS ON, and the audit's central finding was a
   //--- profit factor of 1.126 on generated ticks against 0.853 on real
   //--- ones. From here on, every pass row carries the tape that made it.
   DeclareRunEvidence();
   m_tester.SetProvenance(ProvenanceCsv(),
                          m_prov.CsvColumns(),
                          m_prov.CsvValues());
...
int CProductionEngine::EmergencyFlatten(const string reason)
  {
   ...
   //--- HARNESS v2. An emergency outranks a stand-down: drop the softer
   //--- state so the dashboard and the logs report the terminal condition,
   //--- not a stand-down that will never rearm.
   m_stood_down=false;
...
//+------------------------------------------------------------------+
//| HARNESS v2. Non-terminal counterpart of EmergencyFlatten.            |
//|                                                                      |
//| Suspends new entries for the remainder of the current guard day and   |
//| optionally closes what is open. It deliberately does NOT call         |
//| TriggerEmergencyShutdown: the emergency latch is persisted and clears |
//| only on a human acknowledgement, which is correct for an equity-floor |
//| breach and wrong for a Tuesday.                                      |
//|                                                                      |
//| Open positions remain MANAGED after this returns, exactly as under a  |
//| Halt - ManageOpenPositions runs before the m_running gate.            |
//+------------------------------------------------------------------+
int CProductionEngine::StandDownForDay(const string reason,const bool flatten)
  {
   //--- Already stood down for this day: nothing to do, and re-latching
   //--- would re-close and re-log on every subsequent tick.
   if(m_stood_down)
      return(0);

   m_stood_down     = true;
   m_stand_down_day = (m_limits!=NULL ? m_limits.DayStart()
                                      : CTimeUtils::StartOfDay(TimeCurrent()));
   m_stand_down_count++;
   m_running        = false;
   m_halt_reason    = "stand-down: "+reason;

   int closed=0;
   if(flatten && m_trade!=NULL)
      closed=m_trade.CloseAll(reason);

   if(m_ui!=NULL && m_ui.Logger()!=NULL)
     {
      m_ui.Logger().LogRiskEvent("Engine",
                                 "SESSION STAND-DOWN ("+
                                 IntegerToString(m_stand_down_count)+
                                 "): "+reason+", closed "+
                                 IntegerToString(closed)+
                                 ", rearms after "+
                                 TimeToString(m_stand_down_day,TIME_DATE),
                                 0.0,0.0,0.0);
     }
   return(closed);
  }
//+------------------------------------------------------------------+
//| HARNESS v2. Clears a daily stand-down once the guard has rolled its  |
//| day window AND released its own daily latch.                         |
//|                                                                      |
//| Both conditions are required. The guard rearms m_daily_tripped inside |
//| UpdateWindows, so testing the latch alone would be enough in the      |
//| normal case; the day-start comparison is the belt-and-braces check    |
//| that also covers a weekly or monthly latch still holding, in which    |
//| case the guard keeps refusing entries and the engine must not pretend |
//| it is trading again.                                                 |
//+------------------------------------------------------------------+
void CProductionEngine::RearmIfNewSession(void)
  {
   if(!m_stood_down || m_limits==NULL)
      return;
   //--- An emergency or drawdown latch outranks a stand-down. Never resume
   //--- into one of those.
   if(m_limits.IsEmergencyTripped() || m_limits.IsDrawdownTripped())
      return;
   if(m_limits.DayStart()==m_stand_down_day)
      return;                                   // still the same session
   if(m_limits.IsDailyTripped() || m_limits.IsWeeklyTripped() ||
      m_limits.IsMonthlyTripped())
      return;                                   // a wider window still holds

   m_stood_down     = false;
   m_stand_down_day = 0;
   Resume("new session: daily stand-down expired");
  }
//+------------------------------------------------------------------+
//| HARNESS v2. Folds one broker-fired exit into the aggregates.         |
//|                                                                      |
//| points is signed with POSITIVE = ADVERSE, so the sum is a real cost   |
//| total and favourable fills genuinely offset unfavourable ones instead |
//| of both inflating a magnitude-only average. m_srv_slip_worst tracks   |
//| the worst ADVERSE excursion only - the best fill of the run is not a  |
//| risk figure and does not belong in the same statistic.               |
//+------------------------------------------------------------------+
void CProductionEngine::RecordServerExit(const bool by_stop,
                                         const double points,
                                         const double money)
  {
   m_srv_exits++;
   if(by_stop)
      ...
      m_srv_tp_exits++;
   m_srv_slip_sum   += points;
   m_srv_slip_money += money;
   if(points>m_srv_slip_worst)
      m_srv_slip_worst=points;
  }
//+------------------------------------------------------------------+
//| HARNESS v2. One line of execution truth for the report header.       |
//|                                                                      |
//| The zero case is the important one. If the run had server-side exits  |
//| and every single one filled at exactly its trigger price, that is not |
//| a broker with perfect execution - it is the 1-minute OHLC tick model  |
//| placing the fill on the barrier by construction. Saying so here means |
//| the number can never be quoted as evidence of low slippage.          |
//+------------------------------------------------------------------+
string CProductionEngine::ServerExitSummary(void) const
  {
   if(m_srv_exits<=0)
      return("server-side exits: none observed");
   const double avg=m_srv_slip_sum/(double)m_srv_exits;
   string text=StringFormat("server-side exits: %d (SL %d / TP %d)"
                            " slip avg %+.2f pts, worst %+.2f pts,"
                            " total %+.2f %s [+ = adverse]",
                            (int)m_srv_exits,(int)m_srv_sl_exits,
                            (int)m_srv_tp_exits,avg,m_srv_slip_worst,
                            m_srv_slip_money,AccountInfoString(ACCOUNT_CURRENCY));
   if(ServerFillsAllExact())
      text+="  WARNING: every server fill landed exactly on its trigger."
            " That is the 1-minute-OHLC tick model, not execution quality."
            " Re-run on real ticks before reading any of these figures.";
   return(text);
  }
//+------------------------------------------------------------------+
//| HARNESS v2, FIX 4. The predicate behind that warning, named once.     |
//|                                                                      |
//| Two consumers need it - the printed line above and the provenance     |
//| object's quotable verdict - and if they computed it separately the    |
//| header could carry the warning while still claiming to be quotable.   |
//+------------------------------------------------------------------+
bool CProductionEngine::ServerFillsAllExact(void) const
  {
   if(m_srv_exits<=0)
      return(false);
   return(MathAbs(m_srv_slip_sum)<SRP_EPSILON &&
          MathAbs(m_srv_slip_worst)<SRP_EPSILON);
  }
//+------------------------------------------------------------------+
//| HARNESS v2, FIX 4. The block that opens every report.                |
//|                                                                      |
//| Two things are stitched together here and the seam matters. The       |
//| provenance block says what kind of tape this was; the server-exit     |
//| line says what happened at the barriers. Either one alone is          |
//| misleading - "zero slippage" reads as quality until the line above it |
//| says GENERATED, and "GENERATED" reads as a footnote until the line    |
//| below it shows what that did to the fills. They are printed as one    |
//| unit so that no reader can be handed half of the pair.                |
//|                                                                      |
//| Const, allocation-free and decision-free: it may be called from the   |
//| start path, the shutdown path, or a failed build. It does NOT declare  |
//| the fill evidence - that is a mutation, so Stop() does it immediately  |
//| before printing. See CRunProvenance::SetServerFillEvidence.           |
//+------------------------------------------------------------------+
string CProductionEngine::ProvenanceHeader(void) const
  {
   return(m_prov.Header(ServerExitSummary()));
  }
//+------------------------------------------------------------------+
void CProductionEngine::DeclareRunEvidence(void)
  {
   m_prov.SetServerFillEvidence(m_srv_exits,ServerFillsAllExact());
  }
//+------------------------------------------------------------------+
string CProductionEngine::Describe(void) const
  {
   ...
   text+=" standDowns="+IntegerToString(m_stand_down_count);
   text+="\n  "+ServerExitSummary();
   if(m_decision!=NULL)
      text+="\n  "+m_decision.Describe();
   if(m_risk!=NULL)

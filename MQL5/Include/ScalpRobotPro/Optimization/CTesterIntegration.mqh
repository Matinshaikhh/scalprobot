...
private:
   ...
   //--- HARNESS v2, FIX 4. Provenance, handed down as plain strings by the
   //--- engine. Strings rather than a pointer to the provenance object
   //--- because that object lives in Runtime\ and this layer must not
   //--- depend on it. Empty means "nothing was supplied", which is
   //--- reported rather than quietly producing an unlabelled row.
   string                 m_prov_line;      // one '#' line, for printing
   string                 m_prov_columns;   // CSV column names
   string                 m_prov_values;    // CSV values, same field count
...
public:
   ...
   //--- HARNESS v2, FIX 4. Must be called before Evaluate(), which is where
   //--- the pass row is written. `columns` and `values` are appended to the
   //--- pass CSV as extra fields - PER ROW, because a thousand-pass file is
   //--- sorted by fitness and each row has to carry its own conditions with
   //--- it. See CRunProvenance::CsvColumns.
   void              SetProvenance(const string print_line,
                                   const string columns,
                                   const string values)
     { m_prov_line=print_line; m_prov_columns=columns; m_prov_values=values; }
...
void CTesterIntegration::PublishPassReport(void)
  {
   ...
   //--- HARNESS v2, FIX 4. Above the figures, for the same reason the engine
   //--- prints its block above the run summary.
   if(m_prov_line!="")
      Print(m_prov_line);
   else
      Print("# provenance: NOT SUPPLIED - nothing recorded the tick model; "
            "cost model or data segment behind this pass; quotable=no");
   Print(m_criterion.Explain(m_last_metrics));
  }
//+------------------------------------------------------------------+
string CTesterIntegration::CsvHeader(void) const
  {
   string header="timestamp,symbol,timeframe,fitness,trades,wins,losses,"
                 "win_rate,net_profit,gross_profit,gross_loss,profit_factor,"
                 "expectancy,payoff_ratio,largest_win,largest_loss,"
                 "max_dd_money,max_dd_percent,recovery_factor,sharpe,"
                 "max_con_wins,max_con_losses,max_con_loss_money";
   //--- HARNESS v2, FIX 4. Appended, never inserted: an existing file written
   //--- by an earlier build keeps its column order, and a consumer reading by
   //--- name is unaffected. Whether the provenance fields are present is a
   //--- property of the file, and the header states it.
   if(m_prov_columns!="")
      header+=","+m_prov_columns;
   return(header);
  }
//+------------------------------------------------------------------+
string CTesterIntegration::CsvRow(const SPerformanceMetrics &metrics,
                                  const double fitness) const
  {
   ...
   //--- HARNESS v2, FIX 4. The conditions travel WITH the row, so sorting a
   //--- thousand passes by fitness cannot separate the winner from the tape
   //--- that produced it.
   if(m_prov_values!="")
      row+=","+m_prov_values;
   return(row);

...
bool CDecisionEngine::VoteWeighted(SStrategySignal &winner,
                                   STradeDecision &decision) const
  {
   ...
   const ENUM_SRP_DECISION winning=(buy_weight>sell_weight ? SRP_DECISION_BUY
                                                           : SRP_DECISION_SELL);
   //--- REDESIGN 2026-09-04. The winner is chosen on confidence x weight,
   //--- the same quantity that chose the DIRECTION three lines above.
   //---
   //--- Ranking the direction by weighted contribution and then the carrier
   //--- by raw confidence meant a plugin's weight influenced which way to
   //--- trade but not whose geometry was traded - so a deliberately
   //--- down-weighted plugin could still supply the entry, the stop and the
   //--- suggested invalidation level. Weight is the only lever an operator
   //--- has for saying "trust this plugin less"; it has to apply to both
   //--- halves of the decision or it does not mean anything.
   int best=-1;
   ...
   for(int i=0;i<m_candidate_count;i++)
     {
      if(m_candidates[i].decision!=winning)
         continue;
      const double score=m_candidates[i].confidence*m_candidates[i].weight;

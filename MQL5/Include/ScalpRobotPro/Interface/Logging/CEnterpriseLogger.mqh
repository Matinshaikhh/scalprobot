...
public:
   ...
   //--- the first line - or Printed first, for the Journal format - so an
   //--- exported log states the tick model, cost model and data segment of
   //--- the run that produced it. See CRunProvenance::HeaderCsv.
...
void CEnterpriseLogger::LogTradeClosed(const SClosedTrade &trade)
  {
   ...
   //--- HARNESS v2. Who closed it, and what the fill cost. Written on EVERY
   //--- closed trade rather than only the server-side ones, so an analysis
   //--- can compute the server/EA split from the log without inferring it
   //--- from an absence of rows. Sign convention: + = adverse.
   text+=" by="+(trade.server_side_exit
                 ? (trade.close_reason==(int)DEAL_REASON_SL ? "brokerSL"
                                                            : "brokerTP")
                 : "ea");
   if(trade.server_side_exit)
     {
      text+=" trigger="+DoubleToString(trade.exit_trigger_price,_Digits);
      text+=" exitSlip="+DoubleToString(trade.exit_slippage_points,1)+"pts";
      text+=" exitSlipMoney="+DoubleToString(trade.exit_slippage_money,2);
     }
...
bool CEnterpriseLogger::ExportHistory(const string relative_path,
                                      ...
                                      const string preamble)
  {
   ...
   //--- HARNESS v2, FIX 4. Above the header line in both formats.
   if(preamble!="")

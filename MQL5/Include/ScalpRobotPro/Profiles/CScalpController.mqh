...
struct SScalpVerdict
  {
   ...
   //--- REDESIGN 2026-09-04. The target the EXIT RULE actually collects,
   //--- which is target_points x the early-exit share when that exit is
   //--- armed. Measurement showed 42 of 51 wins in July 2026 closed on the
   //--- early-profit rule and only 5 of 144 trades ever reached the full
   //--- target, so pricing the break-even test on target_points measured a
   //--- geometry the robot never traded. Every figure below now descends
   //--- from this instead.
   double                effective_target_points;
   //--- True when stop_points came from the signal's own invalidation level
   //--- rather than from an ATR multiple.
   bool                  stop_is_structural;
   //--- The win rate the gate actually compared against, and whether it
   //--- was measured from closed trades or asserted by configuration. An
   //--- asserted rate that nothing ever checks is how a losing geometry
   //--- passes inspection.
   double                gate_winrate;
   bool                  gate_winrate_measured;
   ...
   void              Reset(void)
     {
      ...
      effective_target_points=0.0; stop_is_structural=false;
      gate_winrate=0.0; gate_winrate_measured=false;
     }
  };
...
private:
   ...
   //--- HARNESS v2, FIX 4. The spread sample Initialize() derived the cost
   //--- floor and the minimum target from, kept rather than discarded.
   //---
   //--- WHY IT IS STORED: three separate defaults descend from one reading of
   //--- SYMBOL_SPREAD taken at startup, and until now that reading vanished
   //--- the moment Initialize() returned. The report header has to state the
   //--- cost model the run actually used, and a value re-read later is a
   //--- different sample of a moving quantity - on XAUUSD, off-session by a
   //--- factor of three. Reporting a number that was never used to size
   //--- anything would be worse than reporting none.
   //---
   //--- Zero means "Initialize() has not run yet". It is a report-only field:
   //--- nothing downstream sizes, gates or prices from it.
   double            m_init_spread_points;
   //--- WHICH OF THE THREE COST-DERIVED DISTANCES THE CODE CHOSE FOR ITSELF.
   //--- Initialize() overwrites each member only when it arrives at or below
   //--- zero, so once it has run the difference between "the operator asked
   //--- for 34 points" and "the code derived 34 points from one spread
   //--- sample" is unrecoverable - and only the second is invalidated when
   //--- that sample turns out to be unrepresentative of the run.
   bool              m_exec_cost_derived;
   bool              m_target_min_derived;
   bool              m_early_exit_min_derived;
   //--- HARNESS v2, FIX 5. THE DECLARED SPREAD SAMPLE, AND WHETHER IT WON.
   //---
   //--- Unlike m_init_spread_points above, this one is NOT report-only: when
   //--- positive it replaces the sample the three derivations descend from,
   //--- which moves the entry cost gate and the early-exit floor. That is the
   //--- entire point - the three of them descending from an unrepeatable tick
   //--- is what audit 5.5 calls out.
   //---
   //--- 0.0 is the shipping default and means "use the tick", so a build that
   //--- never calls ConfigureSpreadPin behaves exactly as 1.10 did.
   double            m_pin_spread_points;
   bool              m_spread_pinned;
   ...
   //--- REDESIGN 2026-09-04. Closed trades required before the MEASURED
   //--- win rate is allowed to displace the assumed one in the break-even
   //--- test. Below this the sample says nothing and the design figure
   //--- stands.
   int               m_winrate_min_sample;
...
public:
   ...
   void              ConfigureAtrBounds(const double min_points,
                                        const double max_points);
   //--- HARNESS v2, FIX 5. Declares the spread sample Initialize() will use
   //--- instead of the tick. Must be called BEFORE Initialize() or it has no
   //--- effect, which is why the engine calls it beside ConfigureCosts.
   //--- 0 or less leaves the live sample in place.
   void              ConfigureSpreadPin(const double pinned_points);
   ...
   bool              Evaluate(const SScalpSignalId &id,
                              ...
                              const double structural_stop_points=0.0);

   //--- REDESIGN 2026-09-04. The win rate the break-even test compares
   //--- against for a tier: the configured design rate, or the MEASURED
   //--- rate once enough trades have closed to mean anything - whichever
   //--- is lower. See the definition for why the comparison is one-way.
   double            GateWinRate(const ENUM_SRP_SCALP_TIER tier,
                                 bool &measured) const;
   void              SetWinRateMinSample(const int trades)
     { m_winrate_min_sample=(trades<5 ? 5 : trades); }
   ...
   //--- HARNESS v2, FIX 4. The cost model this controller is charging itself,
   //--- from the object that owns the numbers. The engine used to re-read
   //--- SYMBOL_SPREAD to describe it, which answered a slightly different
   //--- question - what the spread is now, not what it was when the defaults
   //--- were derived. Reported, never traded on.
   double            CommissionPoints(void) const { return(m_commission_points); }
   double            ExecutionCostPoints(void) const
     { return(m_execution_cost_points); }
   double            InitSpreadPoints(void) const { return(m_init_spread_points); }
   //--- HARNESS v2, FIX 5. Whether the sample above was declared or sampled.
   //--- The number alone cannot answer that, and the number alone is what
   //--- every report printed until now.
   bool              SpreadPinned(void) const { return(m_spread_pinned); }
   //--- EFFECTIVE, i.e. after Initialize() has filled in whatever was left at
   //--- zero. The configured value and the enforced value are different
   //--- numbers, and a report that prints the configured one describes a run
   //--- that did not happen.
   double            TargetMinPoints(void) const { return(m_target_min_points); }
   double            EarlyExitMinPoints(void) const
     { return(m_early_exit_min_points); }
   double            MinRewardCostRatio(void) const
     { return(m_min_reward_cost_ratio); }
   string            DescribeCostModel(void) const;
...
CScalpController::CScalpController(const string symbol,ILogger *logger)
  : m_symbol(symbol),
    ...
    m_init_spread_points(0.0),
    m_exec_cost_derived(false),
    m_target_min_derived(false),
    m_early_exit_min_derived(false),
    //--- FIX 5. 0.0 / false = the pre-fix behaviour, in every build that
    //--- never wires ConfigureSpreadPin.
    m_pin_spread_points(0.0),
    m_spread_pinned(false),
    m_min_reward_cost_ratio(2.0),
    m_max_spread_target_ratio(0.35),
    //--- REDESIGN 2026-09-04. Matches InpKellyMinTrades: below 30 closed
    //--- trades a measured win rate is noise, not evidence.
    m_winrate_min_sample(30),
    ...
    m_today_day(-1)
  {
   ...
   //=== TIER DEFAULTS =================================================
   //--- These are CONSTRUCTOR defaults. ConfigureTier() overwrites them
   //--- from the EA inputs / profile at startup, but they are kept in
   //--- lockstep with those inputs so the startup coherence check below
   //--- validates the geometry that will actually trade, whatever the call
   //--- order, and so a missing ConfigureTier call cannot resurrect stale
   //--- numbers.
   //---
   //--- GEOMETRY WIDENED 2026-09-05, from cost algebra - NOT a search.
   //--- Two facts forced it:
   //---   1. Round-trip cost on XAUUSD M1 is ~46.5 pts (spread ~34 pinned +
   //---      6.0 commission + ~6.5 execution). A 0.55xATR SUPER target at
   //---      ATR~238 is only ~131 pts, so cost alone was ~35% of gross.
   //---   2. The early-profit exit collects m_early_exit_target_share (0.85)
   //---      of the target, not the target. The gate must therefore clear
   //---      cost against 0.85*target, not target. At the old 0.55/0.45 the
   //---      real, share-and-cost-adjusted break-even sat ABOVE the assumed
   //---      win rate - the tier was admitted on arithmetic about a distance
   //---      the exit rule was configured never to reach.
   //---
   //--- Widened so the ASSUMED rate keeps headroom over the break-even the
   //--- gate recomputes from the NET payoff on the COLLECTED target:
   //---   SUPER    1.35/0.60 M1 : collected 0.85*1.35=1.148xATR; at ATR~238
   //---            net payoff ~1.20, break-even ~45.6%, assumed 0.47.
   //---   STANDARD 1.20/0.60 M1 : held in reserve (regime routing rarely
   //---            selects it); net payoff ~1.15, break-even ~46%, assumed 0.50.
   //---   SWING    1.60/0.75 M5 : collected 0.85*1.60=1.36xATR; at M5 ATR~530
   //---            net payoff ~1.52, break-even ~39.7%, assumed 0.45.
   //---
   //--- Assumed rates are deliberately conservative. GateWinRate() only ever
   //--- LOWERS the admission bar toward the measured rate once >=30 trades
   //--- have closed; it never raises it above these figures, so a tier can
   //--- never trade on an accuracy it has not demonstrated.
   m_tier_enabled[SRP_TIER_SUPER]    = true;
   m_tier_target_atr[SRP_TIER_SUPER] = 1.35;
   m_tier_stop_atr[SRP_TIER_SUPER]   = 0.60;
   m_tier_max_hold[SRP_TIER_SUPER]   = 600;
   m_tier_assumed_winrate[SRP_TIER_SUPER] = 0.47;
   m_tier_timeframe[SRP_TIER_SUPER]  = PERIOD_M1;

   m_tier_enabled[SRP_TIER_STANDARD]    = true;
   m_tier_target_atr[SRP_TIER_STANDARD] = 1.20;
   m_tier_stop_atr[SRP_TIER_STANDARD]   = 0.60;
   m_tier_max_hold[SRP_TIER_STANDARD]   = 300;
   m_tier_assumed_winrate[SRP_TIER_STANDARD] = 0.50;
   m_tier_timeframe[SRP_TIER_STANDARD]  = PERIOD_M1;

   m_tier_enabled[SRP_TIER_SWING]    = true;
   m_tier_target_atr[SRP_TIER_SWING] = 1.60;
   m_tier_stop_atr[SRP_TIER_SWING]   = 0.75;
   m_tier_max_hold[SRP_TIER_SWING]   = 1800;
   m_tier_assumed_winrate[SRP_TIER_SWING] = 0.45;
...
//+------------------------------------------------------------------+
//| REDESIGN 2026-09-04. The win rate the break-even test is entitled  |
//| to assume for a tier.                                              |
//|                                                                    |
//| The assumed rate is a DESIGN CLAIM, not a measurement. SUPER's was  |
//| 0.55 and never checked; the tier ran 35.4% over 144 trades in July  |
//| 2026 and 26.67% in the fix-5 control, and every trade was admitted  |
//| because the gate compared a computed requirement against a constant |
//| nobody had validated.                                              |
//|                                                                    |
//| The comparison is deliberately ONE-WAY: measurement can only LOWER  |
//| the rate the gate will assume, never raise it. A tier running above |
//| its design rate on 40 trades is far more likely to be enjoying a    |
//| favourable stretch than to have discovered a better edge, and       |
//| letting that widen admission is how a good month funds a bad one.   |
//| Underperformance, by contrast, is exactly what the gate exists to    |
//| react to, so it is allowed through immediately.                     |
//+------------------------------------------------------------------+
double CScalpController::GateWinRate(const ENUM_SRP_SCALP_TIER tier,
                                     bool &measured) const
  {
   measured=false;
   const int i=(int)tier;
   if(i<0 || i>2)
      return(1.0);
   const double assumed=m_tier_assumed_winrate[i];
   const long closed=m_tier_wins[i]+m_tier_losses[i];
   if(m_winrate_min_sample<=0 || closed<(long)m_winrate_min_sample)
      return(assumed);
   const double actual=(double)m_tier_wins[i]/(double)closed;
   if(actual>=assumed)
      return(assumed);
   measured=true;
   return(actual);
...
//+------------------------------------------------------------------+
void CScalpController::ConfigureSpreadPin(const double pinned_points)
  {
   //--- Guarded the same way as every other Configure*: a zero or negative
   //--- argument leaves the member alone rather than zeroing it. Here that
   //--- matters more than usual, because a zeroed pin would not disable
   //--- pinning - Initialize() tests for >0, so it would simply fall back to
   //--- the tick, and the report would have to explain why a run that was
   //--- asked to pin did not.
   if(pinned_points>0.0)
      m_pin_spread_points=pinned_points;
  }
//+------------------------------------------------------------------+
...
bool CScalpController::Initialize(void)
  {
   ...
   //--- HARNESS v2, FIX 4. ONE reading of the spread, taken once and kept.
   //--- The three defaults below all descend from it, and each of them used
   //--- to call SymbolInfoInteger for itself. Inside a single Initialize()
   //--- those calls return the same value, so hoisting the read changes no
   //--- number - what it changes is that the value survives the function and
   //--- can be reported. It is sampled unconditionally, including when all
   //--- three settings were supplied explicitly, because the header must be
   //--- able to say what the spread was at startup either way.
   m_init_spread_points=(double)SymbolInfoInteger(m_symbol,SYMBOL_SPREAD);

   //--- HARNESS v2, FIX 5. THE THIRD SAMPLING SITE.
   //---
   //--- The two above are in the classifier and the engine; this one is the
   //--- scalp cost model, and it samples independently of both. Left
   //--- unpinned it makes the execution cost floor, the minimum target and
   //--- the early-exit floor - the three gates that decide whether a signal
   //--- becomes an entry - depend on whichever tick happened to be current
   //--- when the controller was built.
   //---
   //--- Substituted before anything reads it, so the derivations below are
   //--- unchanged in form and simply operate on a declared number.
   if(m_pin_spread_points>0.0)
     {
      m_init_spread_points=m_pin_spread_points;
      m_spread_pinned=true;
     }
   ...
   if(m_execution_cost_points<=0.0)
     {
      m_execution_cost_points=MathMax(m_init_spread_points*0.5,1.0);
      m_exec_cost_derived=true;
     }
   ...
   if(m_early_exit_min_points<=0.0)
     {
      ...
      m_early_exit_min_points=m_init_spread_points+m_commission_points+
                              m_execution_cost_points;
      m_early_exit_min_derived=true;
     }
   m_metrics.Reset();
   m_last_traded.Reset();
   return(true);
   ...
   for(int i=0;i<3;i++)
     {
      ...
      any_tier=true;
      const ENUM_SRP_SCALP_TIER t=(ENUM_SRP_SCALP_TIER)i;
      //--- REDESIGN 2026-09-04. Gross geometry measured on the target the
      //--- early exit lets the trade collect, for the reason set out in
      //--- Evaluate step 6b. Costs are still unknown at startup, so this
      //--- remains the optimistic bound - it just stops being optimistic
      //--- about a distance the exit rule forbids as well.
      const double share=((m_early_exit_enabled &&
                           m_early_exit_target_share>0.0 &&
                           m_early_exit_target_share<1.0)
                          ? m_early_exit_target_share : 1.0);
      const double collected=m_tier_target_atr[i]*share;
      const double gross=(m_tier_stop_atr[i]>0.0
                          ? collected/m_tier_stop_atr[i] : 0.0);
      ...
      if(gross<needed)
         result.AddError(StringFormat(
            ...
            "design win rate - %.2f/%.2f collects %.2f giving payoff %.3f but "
            "%.0f%% wins needs %.3f BEFORE costs",
            TierToString(t),m_tier_target_atr[i],m_tier_stop_atr[i],collected,
            gross,m_tier_assumed_winrate[i]*100.0,needed));
...
bool CScalpController::Evaluate(const SScalpSignalId &id,
                                ...
                                const double structural_stop_points)
  {
   verdict.Reset();
   ...
   //--- THE STOP. Structural when the signal supplied one, ATR-derived
   //--- otherwise.
   //---
   //--- REDESIGN 2026-09-04. CLiquiditySweepStrategy has always computed
   //--- signal.suggested_stop = sweep.extreme_reached with the correct
   //--- reasoning in comment - if price returns beyond the sweep extreme
   //--- the fade thesis is dead - and nothing in the codebase ever read
   //--- the field. The stop was replaced by an ATR multiple, which is a
   //--- distance with no relationship to where the trade is wrong.
   //---
   //--- A supplied level is trusted only inside a band around the ATR
   //--- stop. A stale or mis-scaled level would otherwise size a position
   //--- against a 5-point stop or refuse one against a 5000-point stop,
   //--- and both are worse than the multiple it replaced.
   const double atr_stop=atr_points*m_tier_stop_atr[ti];
   double stop=atr_stop;
   verdict.stop_is_structural=false;
   if(structural_stop_points>0.0)
     {
      const double floor_pts=MathMax(atr_stop*0.40,(double)m_stops_level*1.2);
      const double ceil_pts=atr_stop*1.60;
      if(structural_stop_points>=floor_pts && structural_stop_points<=ceil_pts)
        {
         stop=structural_stop_points;
         verdict.stop_is_structural=true;
        }
     }
   verdict.stop_points=MathMax(stop,(double)m_stops_level*1.2);
   ...
   //--- 6b. THE TARGET THE EXIT RULE ACTUALLY COLLECTS.
   //---
   //--- REDESIGN 2026-09-04. This is the correction that matters most.
   //---
   //--- ShouldTakeEarlyProfit closes a scalp once profit reaches
   //--- m_early_exit_target_share of the target. In July 2026 that rule
   //--- fired on 42 of 51 wins, and only 5 of 144 trades ever touched the
   //--- full target - so every test below this line was being run on a
   //--- distance the robot had been configured never to reach. On the
   //--- shipped gold geometry the gate admitted trades at a stated 55.7%
   //--- break-even whose real break-even, once the 0.85 share is applied,
   //--- was 61.2%. The gate was not wrong about its own arithmetic; it was
   //--- arithmetic about the wrong number.
   //---
   //--- Everything downstream - the reward:cost ratio, the net payoff and
   //--- the required win rate - now prices the exit that fires.
   double effective_target=target;
   if(m_early_exit_enabled &&
      m_early_exit_target_share>0.0 && m_early_exit_target_share<1.0)
     {
      //--- The early exit cannot fire below its own cost floor, so a
      //--- target whose share sits under that floor is still collected in
      //--- full and must not be discounted.
      const double share_points=target*m_early_exit_target_share;
      if(share_points>m_early_exit_min_points)
         effective_target=share_points;
     }
   verdict.effective_target_points=effective_target;
   ...
   //--- Measured against the COLLECTED target, per 6b.
   if(effective_target>0.0 &&
      spread_points>effective_target*m_max_spread_target_ratio)
     {
      verdict.block=SRP_SCALP_BLOCK_SPREAD;
      verdict.detail=StringFormat("spread %.0f pts is %.0f%% of the %.0f pt "
                                  "collected target, ceiling is %.0f%%",
                                  spread_points,
                                  spread_points/effective_target*100.0,
                                  effective_target,
   ...
   //--- 7b. Reward against total modelled cost, on the collected target.
   if(effective_target<cost*m_min_reward_cost_ratio)
     {
      verdict.block=SRP_SCALP_BLOCK_COST;
      m_last_cost_detail=StringFormat("collected target %.0f pts (of %.0f) vs "
                                      "cost %.0f pts (spread %.0f + "
                                      "commission %.0f + execution %.0f), "
                                      "needed %.0f",
                                      effective_target,target,cost,
                                      spread_points,
                                      m_commission_points,
                                      m_execution_cost_points,
                                      cost*m_min_reward_cost_ratio);
      verdict.detail=StringFormat("collected target %.0f pts does not clear "
                                  "cost %.0f pts by the required %.1fx",
                                  effective_target,cost,
   ...
   const double net_win  = effective_target-cost;
   ...
   //--- REDESIGN 2026-09-04. Compared against the rate this tier is
   //--- entitled to assume - measured once the sample is real. See
   //--- GateWinRate.
   bool wr_measured=false;
   const double gate_wr=GateWinRate(tier,wr_measured);
   verdict.gate_winrate=gate_wr;
   verdict.gate_winrate_measured=wr_measured;

   if(net_win<=0.0)
     {
      verdict.block=SRP_SCALP_BLOCK_BREAKEVEN;
      verdict.detail=StringFormat("%s: collected target %.0f pts (of %.0f) "
                                  "does not survive %.0f pts of cost - a win "
                                  "would book a loss",
                                  TierToString(tier),effective_target,
                                  target,cost);
      m_metrics.blocked_cost++;
      m_last_verdict=verdict;
      return(false);
     }
   if(verdict.required_winrate>gate_wr)
     {
      verdict.block=SRP_SCALP_BLOCK_BREAKEVEN;
      verdict.detail=StringFormat("%s: net payoff %.3f needs a %.1f%% win "
                                  "rate but the tier is %s for %.1f%% "
                                  "(collected target %.0f of %.0f, stop %.0f%s,"
                                  " cost %.0f)",
                                  TierToString(tier),verdict.net_payoff,
                                  verdict.required_winrate*100.0,
                                  (wr_measured ? "MEASURED at" : "designed"),
                                  gate_wr*100.0,
                                  effective_target,target,verdict.stop_points,
                                  (verdict.stop_is_structural ? " structural"
                                                              : ""),
                                  cost);
      m_metrics.blocked_cost++;
      m_last_verdict=verdict;
   ...
   verdict.detail=StringFormat("%s: target %.0f pts (collects %.0f), stop "
                               "%.0f pts%s, cost %.0f pts, ATR %.0f pts, net "
                               "payoff %.3f (needs %.1f%% wins vs %.1f%% %s, "
                               "hold %ds)",
                               TierToString(tier),target,effective_target,
                               verdict.stop_points,
                               (verdict.stop_is_structural ? " structural"
                                                           : " ATR"),
...
//+------------------------------------------------------------------+
//+------------------------------------------------------------------+
//| DescribeCostModel. What a round turn is charged, and where each   |
//| number came from.                                                 |
//|                                                                  |
//| HARNESS v2, FIX 4. The provenance header prints these same figures |
//| because the engine hands them over; this is the one place that     |
//| also says which of them the OPERATOR chose and which the CODE      |
//| derived from a single spread reading taken at startup. That        |
//| distinction is what makes the header's "startup spread sample vs   |
//| run average" warning actionable: if nothing was derived, an        |
//| unrepresentative sample cost the run nothing.                      |
//+------------------------------------------------------------------+
string CScalpController::DescribeCostModel(void) const
  {
   if(m_init_spread_points<=0.0 && !m_exec_cost_derived)
      return("SCALP COST MODEL: not initialised");

   string text=StringFormat("SCALP COST MODEL: commission %.2f pts/round-turn, "
                            "execution %.2f pts (%s)",
                            m_commission_points,m_execution_cost_points,
                            (m_exec_cost_derived ? "derived" : "configured"));
   text+=StringFormat("\n  startup spread sample: %.2f pts (%s)",
                      m_init_spread_points,
                      //--- FIX 5. The sample and its provenance on one line:
                      //--- the number is only interpretable with this beside
                      //--- it, and a run that cannot be repeated should say so
                      //--- everywhere it quotes the figure.
                      (m_spread_pinned ? "PINNED, reproducible"
                                       : "live tick, NOT reproducible"));
   text+=StringFormat("\n  target floor %.2f pts (%s), early-exit floor %.2f "
                      "pts (%s), min reward/cost %.2f",
                      m_target_min_points,
                      (m_target_min_derived ? "derived" : "configured"),
                      m_early_exit_min_points,
                      (m_early_exit_min_derived ? "derived" : "configured"),
                      m_min_reward_cost_ratio);
   //--- Named rather than counted: "3 distances derived" would still leave the
   //--- reader to work out which ones move when the sample is wrong.
   const int derived=(m_exec_cost_derived ? 1 : 0)+
                     (m_target_min_derived ? 1 : 0)+
                     (m_early_exit_min_derived ? 1 : 0);
   if(derived>0)
      text+=StringFormat("\n  %d of the 3 above descend from that one spread "
                         "sample; a sample unrepresentative of the run "
                         "mis-sizes every one of them.",derived);
   else
      text+="\n  all three were configured explicitly, so the startup spread "
            "sample sized nothing.";
   return(text);
  }
//+------------------------------------------------------------------+

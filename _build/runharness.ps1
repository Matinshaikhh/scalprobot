...
param(
    ...
    # Which tree the harness lives in. Added because the verdict pattern below
    # now includes XSP_CHECK, and a token that no reachable code path can emit
    # is worse than no token: it reads as coverage that does not exist.
    # ScalpRobotPro's includes are deployed either way - XSP includes
    # CRunProvenance from it - so the two trees never need separate runs.
    [ValidateSet('ScalpRobotPro', 'XauStructurePro')]
    [string]$Tree    = 'ScalpRobotPro',
    [string]$Symbol  = 'EURUSD',
    # Every token that actually terminates a harness with a verdict line, in
    # both trees. Enumerated from the sources rather than assumed: the old
    # default listed three of the eight, so SRP_SCALP, SRP_NEWSGATE,
    # SRP_SESSIONGATE, SRP_VOLUME and XSP_CHECK could all PASS and still be
    # reported as "NO VERDICT LINE FOUND" - a false negative indistinguishable
    # from a real failure.
    #
    # Verdict tokens ONLY. XSP_STUDY, XSP_TRACK and friends prefix diagnostics,
    # not verdicts, and $Pattern is consumed by `Select-Object -Last 1` - so
    # including one would let a stray note be picked up AS the verdict.
    [string]$Pattern = 'SRP_RESULT|SRP_PROFILE|SRP_REGIME|SRP_SCALP|SRP_VOLUME|SRP_NEWSGATE|SRP_SESSIONGATE|XSP_CHECK',
...
# Copying a tree into the MT5 runtime is a DEPLOYMENT, not a modification: the
# audit baseline is the copy under 'c:\scalp robot\MQL5', which is only read.
if ($Tree -eq 'XauStructurePro') {
    $xspInc = Join-Path $mqlRt 'Include\XauStructurePro'
    if (Test-Path $xspInc) { Remove-Item $xspInc -Recurse -Force }
    Copy-Item (Join-Path $src 'Include\XauStructurePro') `
              -Destination (Join-Path $mqlRt 'Include') -Recurse -Force
    $xspExp = Join-Path $mqlRt 'Experts\XauStructurePro'
    if (Test-Path $xspExp) { Remove-Item $xspExp -Recurse -Force }
    Copy-Item (Join-Path $src 'Experts\XauStructurePro') `
              -Destination (Join-Path $mqlRt 'Experts') -Recurse -Force
}

# --- compile the harness in place ---------------------------------------
$ea  = Join-Path $mqlRt "Experts\$Tree\$Harness.mq5"
if (-not (Test-Path $ea)) { Write-Output "NO SUCH HARNESS: $Tree\$Harness"; exit 1 }

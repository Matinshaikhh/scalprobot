...
# HARNESS v2, FIX 4. The EA now writes a '#'-prefixed provenance line as the
# first line of every export: tick model, cost model, in/out-of-sample status.
# It is ECHOED rather than merely skipped - the whole point of stamping it into
# the file is that a downstream table cannot be read without it - and then
# dropped before parsing, because it has a different field count from the rows.
$lines = @(Get-Content -LiteralPath $Csv)
foreach ($l in $lines) { if ($l -like '#*') { $l } }
$rows = $lines | Where-Object { $_ -notlike '#*' -and $_.Trim() -ne '' } |
        ConvertFrom-Csv
$rows = @($rows)
"rows: $($rows.Count)"

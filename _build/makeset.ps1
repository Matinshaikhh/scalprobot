...
$enumOrdinals = @{
    ...
    # HARNESS v2, FIX 4. The declared in/out-of-sample status. Mapped here as
    # well as in source because the .set file is how a scripted run declares
    # it: without these three the new input would arrive as the name and be
    # read as 0, i.e. every scripted OOS run would silently report itself as
    # UNDECLARED - the exact omission fix 4 exists to close.

...
input bool     InpBosEnabled               = true;       // Strategy: break of structure
...
input double   InpScalpCommissionPoints    = 0.0;        // Commission (points per round trip)
...
input double   InpScalpEarlyExitTargetShare= 0.55;       // Early exit at share of target
...
//--- Target EXCEEDS stop deliberately. A 0.30/0.42 geometry needs 58.4%
//--- wins before costs; the measured rate with the accuracy filter active
//--- is 52.5%, so that geometry could not be paid for. 0.55/0.45 breaks
//--- even at 45.0% gross, which 52.5% clears with margin.
input double   InpTierSuperTargetAtr       = 0.65;       // SUPER target (ATR multiple)
input double   InpTierSuperStopAtr         = 0.45;       // SUPER stop (ATR multiple)
input int      InpTierSuperHoldSeconds     = 120;        // SUPER max hold (s)
//--- Set BELOW the measured 52.5%, never above it: this figure is what
//--- authorises a small target, so it must be earned rather than claimed.
input double   InpTierSuperWinRate         = 0.55;       // SUPER design win rate
...
input double   InpTierStandardTargetAtr    = 0.70;       // STANDARD target (ATR multiple)
input double   InpTierStandardStopAtr      = 0.70;       // STANDARD stop (ATR multiple)
input int      InpTierStandardHoldSeconds  = 300;        // STANDARD max hold (s)
input double   InpTierStandardWinRate      = 0.58;       // STANDARD design win rate
...
input double   InpTierSwingTargetAtr       = 1.40;       // SWING target (ATR multiple)
input double   InpTierSwingStopAtr         = 0.90;       // SWING stop (ATR multiple)
input int      InpTierSwingHoldSeconds     = 900;        // SWING max hold (s)
input double   InpTierSwingWinRate         = 0.48;       // SWING design win rate
